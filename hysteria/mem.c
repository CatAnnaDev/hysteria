...
int mem_ok(const void *p, int n) {
  ...
  if (mbi.Protect & PAGE_GUARD)
    return 0;
  unsigned int base = (unsigned int)(ULONG_PTR)mbi.BaseAddress,
...
static int entry_name_off(void *entry, const char *want) {
  ...
  for (unsigned k = 0; k < sizeof(offs) / sizeof(offs[0]); k++) {
    ...
    if (mem_ok(nm, 8) && lstrcmpA(nm, want) == 0)
      return offs[k];
  }
  return -1;
}
...
void scan_gnames(void) {
  ...
  while (a < 0x20000000 && VirtualQuery((void *)a, &mbi, sizeof mbi)) {
    ...
    if (rd && sz >= 12 && sz < 0x4000000) {
      for (unsigned int p = base; p + 12 <= base + sz; p += 4) {
        ...
        if (cnt < 200 || cnt > 400000 || mx < cnt || mx > cnt * 4 + 16)
          continue;
        void **data = *(void ***)p;
        if (!mem_ok(data, 8))
          continue;
  ...
  if (!warned) {
    warned = 1;
    logmsg("[hysteria] GNames not ready (cand=%d dataok=%d), retrying...\r\n",
           cand, dataok);
  }
}
...
const char *uname(int id) {
  if (!g_gnames || id < 0)
    return "<no-gnames>";
  void **gn = *(void ***)g_gnames;
  int cnt = *(int *)(g_gnames + 4);
  if (!mem_ok(gn, 8) || id >= cnt)
    return "<?>";
  void *entry = gn[id];
  if (!mem_ok(entry, g_nameOff + 2))
    return "<?>";
...
void probe_engine_offsets(void) {
  ...
  if (cnt > 20000)
    cnt = 20000;
  int nameCount =
      mem_ok((void *)(ULONG_PTR)g_gnames, 8) ? *(int *)(g_gnames + 4) : 0;
  if (nameCount < 10)
    return;
  static const int nameC[] = {0x2C, 0x28, 0x30, 0x24, 0x20, 0x34,
                              0x0C, 0x10, 0x14, 0x18, 0x1C, 0x38};
  static const int classC[] = {0x34, 0x30, 0x2C, 0x28, 0x38, 0x3C,
                               0x40, 0x24, 0x44, 0x48, 0x20};
  ...
  for (unsigned ai = 0; ai < sizeof nameC / sizeof *nameC; ai++)
    for (unsigned bi = 0; bi < sizeof classC / sizeof *classC; bi++) {
      ...
      if (oN == oC)
        continue;
      int wide = (oN > oC ? oN : oC) + 8, hits = 0, tries = 0,
          step = cnt / 300 + 1;
      for (int i = 0; i < cnt && tries < 300; i += step) {
        ...
        if (ci < 0 || ci >= nameCount)
          continue;
        const char *nm = uname(ci);
        if (nm && lstrcmpA(nm, "Class") == 0)
          hits++;
      }
      if (hits > bestHits) {
        bestHits = hits;
  ...
  if (bestHits >= 8) {
    ...
    for (unsigned k = 0; k < sizeof outC / sizeof *outC; k++) {
      ...
      for (int i = 0; i < cnt && tr < 120; i += step) {
        ...
        if (mem_ok(ou, g_oName + 4)) {
          int ni = *(int *)((char *)ou + g_oName);
          if (ni >= 0 && ni < nameCount)
            ok++;
        }
      }
      if (tr > 0 && ok >= tr * 9 / 10) {
    ...
    logmsg("[hysteria] engine offsets probed: name=0x%x class=0x%x outer=0x%x "
           "(hits=%d)\r\n",
...
static LONG CALLBACK hysteria_veh(EXCEPTION_POINTERS *ep) {
  if (g_scanning && GetCurrentThreadId() == g_scanTid &&
      ep->ExceptionRecord->ExceptionCode == (DWORD)0xC0000005) {
    longjmp(g_sj, 1);
  }
...
void scan_init(void) {
  ...
  if (!done) {
    ...
    AddVectoredExceptionHandler(1, (PVECTORED_EXCEPTION_HANDLER)hysteria_veh);
  }
}
...
static void scan_setup_skips(void) {
  ...
  GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                     ...
                     (LPCSTR)&scan_setup_skips, &self);
  g_selfLo = (unsigned int)(ULONG_PTR)self;
  g_selfHi = g_selfLo + 0x900000;
  MEMORY_BASIC_INFORMATION smb;
  int local = 0;
  if (VirtualQuery(&local, &smb, sizeof smb))
    g_stackBase = (unsigned int)(ULONG_PTR)smb.BaseAddress;
}
static int skip_region(unsigned int base) {
  if (base >= 0x70000000)
    return 1;
  if (base >= g_selfLo && base < g_selfHi)
    return 1;
  if (g_stackBase && base == g_stackBase)
    return 1;
  return 0;
}
...
void dump_name_layout(const char *s) {
  ...
  unsigned int hit = 0;
  while (a < 0x7f000000 &&
         VirtualQuery((void *)(ULONG_PTR)a, &mbi, sizeof mbi)) {
    unsigned int base = (unsigned int)(ULONG_PTR)mbi.BaseAddress,
                 sz = mbi.RegionSize;
    if (!sz)
      break;
    ...
    int rd = (mbi.State == MEM_COMMIT) && !(mbi.Protect & PAGE_GUARD) &&
             (pr == PAGE_READWRITE || pr == PAGE_READONLY ||
              pr == PAGE_WRITECOPY || pr == PAGE_EXECUTE_READ);
    if (rd && !skip_region(base) && sz < 0x8000000 && found < 2) {
      for (unsigned int pg = base; pg + 0x1000 <= base + sz && found < 2;
           pg += 0x1000) {
        if (setjmp(g_sj)) {
          g_scanning = 0;
          continue;
        ...
        for (unsigned int p = pg; p + slen + 1 <= pg + 0x1000; p++) {
          ...
          logmsg("[hysteria][diag] '%s' @%08x  pre16: %08x %08x %08x %08x\r\n",
                 s, p, *(unsigned *)(ULONG_PTR)h0,
                 *(unsigned *)(ULONG_PTR)(h0 + 4),
                 *(unsigned *)(ULONG_PTR)(h0 + 8),
                 *(unsigned *)(ULONG_PTR)(h0 + 12));
          break;
        }
        g_scanning = 0;
      }
  ...
  if (!hit) {
    logmsg("[hysteria][diag] '%s' not found\r\n", s);
    return;
  }
  a = 0x10000;
  int ptrs = 0;
  while (a < 0x7f000000 &&
         VirtualQuery((void *)(ULONG_PTR)a, &mbi, sizeof mbi) && ptrs < 4) {
    unsigned int base = (unsigned int)(ULONG_PTR)mbi.BaseAddress,
                 sz = mbi.RegionSize;
    if (!sz)
      break;
    ...
    if (rd && !skip_region(base) && sz < 0x8000000) {
      for (unsigned int pg = base; pg + 0x1000 <= base + sz && ptrs < 4;
           pg += 0x1000) {
        if (setjmp(g_sj)) {
          g_scanning = 0;
          continue;
        ...
        for (unsigned int p = pg; p + 4 <= pg + 0x1000; p += 4) {
          ...
          if (v == hit || (hit >= 16 && v == hit - 16) ||
              (hit >= 12 && v == hit - 12) || (hit >= 8 && v == hit - 8)) {
...
static int looks_like_object(void *o) {
  ...
  for (unsigned k = 0; k < sizeof nc_off / sizeof *nc_off; k++) {
    ...
    if (idx > 0 && idx < nc) {
      const char *nm = uname(idx);
      if (mem_ok(nm, 2) && nm[0] > 32 && nm[0] < 127)
        return 1;
    }
  }
  return 0;
...
void scan_gobjects(void) {
  if (g_gobjects) {
    ...
    if (mem_ok(data, 8)) {
      int cnt = *(int *)(g_gobjects + 4);
      if (cnt > 100 && cnt < 2000000 && mem_ok(data[0], 0x40) &&
          looks_like_object(data[0]))
        return;
    }
  }
  MEMORY_BASIC_INFORMATION mbi;
  ...
  while (a < 0x40000000 && VirtualQuery((void *)a, &mbi, sizeof mbi)) {
    ...
    if (rd && sz >= 12 && sz < 0x4000000) {
      for (unsigned int p = base; p + 12 <= base + sz; p += 4) {
        int cnt = *(int *)(p + 4), mx = *(int *)(p + 8);
        if (cnt < 200 || cnt > 2000000 || mx < cnt || mx > cnt * 2 + 1024)
          continue;
        void **data = *(void ***)p;
        if (!mem_ok(data, 32))
          continue;
        int good = 0;
        for (int k = 0; k < 8; k++)
          if (mem_ok(data[k], 0x40) && looks_like_object(data[k]))
            good++;
        if (good >= 6) {
          g_gobjects = p;
          logmsg("[hysteria] GObjects scanned @%08x cnt=%d\r\n", p, cnt);
          return;
        }
      }
    }
...
int find_name(const char *s) {
  ...
  for (int i = 0; i < cnt; i++) {
    void *e = gn[i];
    if (!mem_ok(e, g_nameOff + 2))
      continue;
    if (lstrcmpA((const char *)e + g_nameOff, s) == 0)
      return i;
  }
  return -1;
}
const char *obj_class_name(void *o) {
  if (!mem_ok(o, O_CLASS + 4))
    return "?";
  void *cls = *(void **)((char *)o + O_CLASS);
  if (!mem_ok(cls, O_NAME + 4))
    return "?";
...
void find_pc(void) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    if (!mem_ok(o, O_CLASS + 4))
      continue;
    if (!contains(obj_class_name(o), "PlayerController"))
      continue;
    const char *on = obj_name(o);
    if (on[0] == 'D' && on[1] == 'e' && on[2] == 'f')
      continue;
    g_pc = o;
    return;
  }

class ASM_JumpPadPhysics extends AliceSpecialMove
    native
    notplaceable;
...
event PlayFall()
{
    ...
    {
        PawnOwner.EndSpecialMove();
        PCOwner.bJumpFromJumpPad = true;
        if (AlicePlayerInput(PlayerController(PawnOwner.Controller).PlayerInput).IsKeyPressed('XboxTypeS_A'))
        {
            PawnOwner.DoSpecialMove(3, false);
            AlicePawn(PawnOwner).bFloatDown = true;
            AlicePawn(PawnOwner).bIsDoubleJumping = true;
            PawnOwner.SetPhysics(17);
            AlicePlayerController(PawnOwner.Controller).GotoState('PlayerFloat');
        }
        else
        {
            PawnOwner.SetPhysics(2);
            AlicePlayerController(PawnOwner.Controller).GotoState('PlayerWalking');
            PawnOwner.DoSpecialMove(3, false);
            AlicePawn(PawnOwner).bIsDoubleJumping = true;
        }
        AlicePlayerController(PawnOwner.Controller).CycleFloatManager.jumpPadInit();
    }
}
...
function StopRise()
{
    ...
    AlicePawn(PawnOwner).bJumpPadHeightestPositionKeep = false;
}
...
function PlayRise()
{
    ...
    AlicePawn(PawnOwner).bJumpPadHeightestPositionKeep = true;
}
